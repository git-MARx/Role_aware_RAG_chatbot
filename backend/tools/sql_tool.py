...
def sql_tool_node(state: GraphState) -> dict:
    ...
    if category in ("personal", "someone_else") and data_type is None:
        return {"sql_result": "I don't have access to this information yet."}

    access = check_access(role, category)

    if access == "denied":
        return {"sql_result": "You don't have permission to view this information."}
    ...
    if result is None:
        return {"sql_result": "No data found for your query."}
...
if __name__ == "__main__":
    ...
    meera = _get_emp_id_by_name("Meera Krishnan") # senior hr
    ...
    for label, state in cases:
        ...
        print(f"{label}")
        print(f"  sql_result: {result['sql_result']}")
        print("-" * 60)
